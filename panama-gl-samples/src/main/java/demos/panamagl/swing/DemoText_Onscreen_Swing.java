 ...
 *
 */
public class DemoText_Onscreen_Swing {
  ...
  public static void main(String[] args) throws Exception {
    ...
    // Create frame
    final JFrame frame = new JFrame(DemoText_Onscreen_Swing.class.getSimpleName());
  ...
  public static GLEventAdapter Texts() {
    return new GLEventAdapter() {
      ...
      public void display(final GL gl) {
        ...
        TextRenderer<Font,Color> txt = new BasicTextRenderer();
        ...
        for (float i = -1; i <= 1; i+=step) {
          for (float j = -1; j <= 1; j+=step) {
            float z = i*j;
            String label = "x=" + i + " z=" + z;
            txt.draw(gl, f, label, i, j, z, c, 0);
          }
        }
