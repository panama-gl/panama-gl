...
public class CGLContext implements GLContext {
  ...
  public synchronized void init() {
    MemorySegment npix = CGLChoosePixelFormat();

    CGLCreateContext(npix);
  ...
  public synchronized void makeCurrent() {
    ...
    /*
     * CGLSetCurrentContext is a function in the CGL (Core Graphics Library) API that sets the
     * current graphics context for the calling thread. It is used to specify the graphics context
     * that should be used for rendering or other graphics operations in the current thread.
     */
    err = OpenGL_h.CGLSetCurrentContext(context);
    ...
    if (OpenGL_h.kCGLNoError() == err) {
      ...
      System.err.println("CGLContext : set current : " + err + " = kCGLBadContext(). The specified context is invalid.");

    } 
    else if (OpenGL_h.kCGLBadConnection() == err) {
      System.err.println("CGLContext : set current : " + err + " = kCGLBadConnection(). The specified context is not connected to a window or drawable.");
  ...
  protected void CGLGetCurrentContext() {
  ...
  protected void CGLCreateContext(MemorySegment npix) {
  ...
  protected MemorySegment CGLChoosePixelFormat() {
    int[] at = new int[4];
